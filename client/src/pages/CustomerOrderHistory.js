...
const CustomerOrderHistory = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return statuses[status] || 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300';
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t('orderHistory')}</h1>
          <p className="text-gray-500 dark:text-gray-400">{t('enterPhoneToViewOrders')}</p>
        </div>
          ...
          <>
            ...
            {orders.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 text-center border border-gray-200 dark:border-gray-700 shadow-sm">
                <Receipt size={48} className="mx-auto text-gray-400 dark:text-gray-600 mb-3" />
              ...
              <div className="space-y-4">
                ...
                {orders.map(order => (
                  <div key={order.id} className="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all shadow-sm hover:shadow-md">
                      ...
                      <div className="border-t border-gray-100 dark:border-gray-700 pt-3 mt-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {order.items.slice(0, 3).map((item, idx) => (
                            <span key={idx} className="text-gray-700 dark:text-gray-300 text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg">
                              {item.quantity}x {item.product_name}
                            </span>
                          ))}
                          {order.items.length > 3 && (
                            <span className="text-gray-500 dark:text-gray-400 text-sm">+{order.items.length - 3} {t('more')}</span>
                          )}
                        </div>
                        <button
                          onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full max-h-[80vh] overflow-y-auto border border-gray-200 dark:border-gray-700">
            <div className="sticky top-0 bg-white dark:bg-gray-800 p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              ...
              <button onClick={() => setShowDetailsModal(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">✕</button>
            </div>
            <div className="p-4 space-y-4">
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
                <p className="text-gray-500 dark:text-gray-400 text-xs">{t('orderNumber')}</p>
                <p className="text-gray-900 dark:text-white font-bold text-lg">{selectedOrder.order_number}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3">
                <p className="text-gray-500 dark:text-gray-400 text-xs">{t('totalAmount')}</p>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
                <div className="flex justify-between items-center">
                  <span className="text-gray-500 dark:text-gray-400">{t('orderDate')}</span>
                  <span className="text-gray-700 dark:text-gray-300 text-sm">{formatDate(selectedOrder.created_at)}</span>
                </div>
              </div>
            </div>
          </div>
