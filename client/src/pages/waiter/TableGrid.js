  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <div className="p-3 md:p-8 space-y-4 md:space-y-6 pb-24 md:pb-8">
          ...
          <div className="p-4">
            ...
            {myAssignedTables.length >= 5 && (
              <div className="mt-3 p-2 bg-yellow-500/10 rounded-lg border border-yellow-500/30">
                <p className="text-yellow-400 text-xs text-center">You have reached the maximum of 5 tables.</p>
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                        ...
                        <span className="text-gray-400 text-xs md:text-sm">{t('table')} {order.table_number}</span>
                      </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => {
                    ...
                    if (canvas) {
                      ...
                      printWindow.document.write(`<html><head><title>QR Code</title><style>body{display:flex;justify-content:center;align-items:center;height:100vh;margin:0;padding:20px}.qr-container{text-align:center}img{max-width:300px}</style></head><body><div class="qr-container"><img src="${canvas.toDataURL()}" /><p>Table ${qrTable.table_number}</p></div></body></html>`);
        ...
        {/* NEW ORDER MODAL - Simplified for brevity, keep existing working code */}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
            <div className="bg-gray-800 rounded-2xl md:rounded-3xl w-full max-w-5xl max-h-[95vh] overflow-hidden border border-gray-700 shadow-2xl flex flex-col">
              <div className="flex-shrink-0 sticky top-0 bg-gray-800/95 backdrop-blur-sm p-3 md:p-5 border-b border-gray-700 flex justify-between items-center z-10">
                ...
                <button onClick={() => { setShowOrderModal(false); setCart([]); setSelectedTable(null); }} className="w-8 h-8 rounded-full bg-gray-700 hover:bg-gray-600 text-white flex items-center justify-center">✕</button>
              </div>
                  ...
                  <div className="flex-1">
                    <div className="grid grid-cols-2 gap-2">
                      {filteredProducts.slice(0, 20).map(product => (
                        <button key={product.id} onClick={() => addToCart(product)} className="bg-gray-700/50 hover:bg-gray-700 rounded-lg p-2 text-left">
                          <p className="text-white font-semibold text-sm">{product.name}</p>
                          <p className="text-emerald-400 font-bold text-sm">Br {parseFloat(product.price).toFixed(2)}</p>
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className="w-full lg:w-96 bg-gray-700/30 rounded-xl p-3">
                    <h3 className="text-white font-semibold mb-3">Current Order</h3>
                    {cart.map(item => (
                      <div key={item.id} className="bg-gray-800 rounded-lg p-2 mb-2">
                    ...
                    {cart.length > 0 && (
                      <button onClick={submitOrder} disabled={isSubmitting} className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold mt-3">
        ...
        {showCancelModal && orderToCancel && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-xl w-full max-w-md">
              <div className="p-4">
                <h2 className="text-lg font-bold text-white mb-4">Cancel Order</h2>
                <p className="text-gray-300 text-sm">Order: #{orderToCancel.order_number}</p>
                <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="Reason for cancellation" className="w-full px-3 py-2 bg-gray-700 rounded-lg text-white mt-3" rows={3} />
