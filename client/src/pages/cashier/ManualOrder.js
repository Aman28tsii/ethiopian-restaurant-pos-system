  ...
  if (orderComplete) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="bg-white dark:bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-md w-full text-center border border-gray-200 dark:border-gray-200 dark:border-gray-700 shadow-lg">
          <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle size={40} className="text-green-600 dark:text-green-400" />
          </div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-900 dark:text-white mb-2">{t('orderComplete')}</h2>
          <p className="text-gray-500 dark:text-gray-500 dark:text-gray-400 mb-4">{t('orderSentKitchen')}</p>
          ...
          <button
            ...
            className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-gray-900 dark:text-white rounded-xl font-semibold transition"
          >
            {t('newOrder')}
          </button>
        </div>
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 min-w-0">
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-900 dark:text-white">{t('manualOrder')}</h1>
          <p className="text-gray-500 dark:text-gray-500 dark:text-gray-400 mt-1">{t('createManualOrder')}</p>
        </div>

        {/* Order Type Selector */}
        <div className="bg-white dark:bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-200 dark:border-gray-700 mb-4">
          <div className="flex gap-4 mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-gray-700 dark:text-gray-600 dark:text-gray-300">{t('dineIn')}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                ...
                className="w-4 h-4 text-blue-600"
              />
              <span className="text-gray-700 dark:text-gray-600 dark:text-gray-300">{t('takeaway')}</span>
            </label>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {filteredProducts.map(product => (
            <button
              ...
              className="bg-white dark:bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-200 dark:border-gray-700 rounded-xl p-3 text-left hover:border-blue-500 transition hover:shadow-md"
            >
              <div className="text-4xl text-center mb-2">{getProductEmoji(product.category)}</div>
              <h3 className="font-semibold text-gray-900 dark:text-gray-900 dark:text-white text-sm mb-1 line-clamp-2">{product.name}</h3>
      ...
      {/* Right Panel - Cart */}
      <div className="w-full lg:w-96 bg-white dark:bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-200 dark:border-gray-700 flex flex-col h-[calc(100vh-120px)] sticky top-6 shadow-lg">
        <div className="p-4 border-b border-gray-200 dark:border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-900 dark:text-white">{t('currentOrder')}</h2>
            </div>
            {cart.length > 0 && (
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-500 dark:text-gray-400">{t('cartEmpty')}</p>
              <p className="text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">{t('tapToAdd')}</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.id} className="bg-gray-50 dark:bg-gray-100 dark:bg-gray-700 rounded-xl p-3">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-gray-900 dark:text-white">{item.name}</h3>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className="text-gray-900 dark:text-gray-900 dark:text-white font-bold">{formatCurrency(item.total)}</span>
                </div>
              </div>
            ))
          )}
        ...
        <div className="border-t border-gray-200 dark:border-gray-200 dark:border-gray-700 p-4 space-y-3">
          <input
            type="text"
            placeholder={t('customerNameOptional')}
            value={customerName}
            onChange={(e) => setCustomerName(e.target.value)}
            className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <input
            type="tel"
          ...
          <div>
            <p className="text-gray-600 dark:text-gray-500 dark:text-gray-400 text-sm mb-2">{t('paymentMethod')}</p>
          ...
          <div className="pt-2 border-t border-gray-200 dark:border-gray-200 dark:border-gray-700">
            <div className="flex justify-between text-gray-600 dark:text-gray-500 dark:text-gray-400 text-sm">
            ...
            <div className="flex justify-between text-gray-900 dark:text-gray-900 dark:text-white font-bold text-lg pt-2">
              <span>{t('total')}</span>
          ...
          <button
            ...
            className="w-full py-3 bg-green-600 hover:bg-green-700 text-gray-900 dark:text-white rounded-xl font-bold transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
