...
const MyOrders = () => {
  ...
  useEffect(() => {
    ...
    socket.on('order_status_updated', (data) => {
      ...
      if (data.status === 'ready') {
        setNotification(`🍽️ ${t('order')} #${data.order_id} ${t('isNow')} ${t('ready')}!`);
    ...
    socket.on('new_order', (data) => {
      fetchOrders();
      setNotification(`📋 ${t('newOrderReceived')} #${data.order_number}!`);
      setTimeout(() => setNotification(null), 5000);
    });
  ...
  const getStatusBadge = (status) => {
    const statuses = {
      'confirmed': { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-400', label: t('confirmed'), icon: <CheckCircle size={14} /> },
      'pending': { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-400', label: t('inKitchen'), icon: <Clock size={14} /> },
      'preparing': { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-700 dark:text-orange-400', label: t('preparing'), icon: <ChefHat size={14} /> },
      'ready': { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-700 dark:text-green-400', label: t('readyToServe'), icon: <Coffee size={14} /> },
      'completed': { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-700 dark:text-purple-400', label: t('completed'), icon: <CheckCircle size={14} /> }
    };
    return statuses[status] || { bg: 'bg-gray-100 dark:bg-gray-100 dark:bg-gray-700', text: 'text-gray-700 dark:text-gray-600 dark:text-gray-300', label: status, icon: <Clock size={14} /> };
  };
