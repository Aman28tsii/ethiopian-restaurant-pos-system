...
const KitchenDashboard = () => {
  ...
  const updateStatus = async (orderId, status) => {
    try {
      await API.put(/kitchen/orders//status, { status });
      fetchOrders();
    } catch (err) {
  ...
  const getStatusButton = (status, orderId) => {
    ...
    if (status === 'ready') {
      return (
        <div className="w-full py-3 sm:py-4 bg-gray-100 dark:bg-gray-700 text-green-600 dark:text-green-400 rounded-xl text-sm sm:text-base font-bold text-center">
          🍽️ {t('readyForPickup')}
        </div>
      );
    }
